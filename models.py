...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 base_model='tiny_resnet50', new_length=None,
    ...
    TSN Configurations:
        ...
        # core code
        if consensus_type in ['TRN', 'TRNmultiscale', 'TRNmultiscaleWithClassifier']:
    ...
    # 根据不同的需求对backbone进行修饰，以满足TRN网络的特殊要求
    def _prepare_tsn(self, num_class):
        # 卷积神经网络输出特征的维度
        feature_dim = 512
        self.avgpool = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        # 在对特征做关系推理之前，TRN 要求接dropout layer
        self.dropout = nn.Dropout(p=self.dropout)

        if self.consensus_type in ['TRN', 'TRNmultiscale', 'TRNmultiscaleWithClassifier']:
    ...
    # 由于pytorch版本更新的问题，_prepare_base_model需要使用TSM中相应函数替换
    ...
    # freeze the BN parameters，这种trick知道就行，管他怎么实现
    ...
    def forward(self, input):
        ...
        # input size：(N, C, num_segments, W, H) --> (N x num_segments, channel, w, h)
        base_out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]))[3]
        # (N x num_segments, channel, w, h) --> (N x num_segments, channel)
        base_out = self.avgpool(base_out).squeeze()
        base_out = self.dropout(base_out)
        print("base_out size: {}".format(base_out.shape))
        # 对卷积神经网络得到的特征稍作微调
        base_out = self.new_fc(base_out)

        # (N x num_segments, channel) --> (N, num_segments, channel)
        if self.reshape:
            base_out = base_out.view((-1, self.num_segments) + base_out.size()[1:])

        # (N, num_segments, channel) --> (N, num_class)
...
if __name__ == '__main__':
    tsn = TSN(174, 8, "RGB",
             base_model='tiny_resnet50', new_length=None,
             consensus_type='TRNmultiscale', before_softmax=True,
             dropout=0.8, img_feature_dim=256,
             crop_num=1, partial_bn=True, print_spec=True, pretrain='imagenet')
    # input size：(N, C, num_segments, W, H)
